...
public class CarreraControlador {// No lo estoy usando
	...
	public static Carrera crearCarreraDragster(ArrayList<Garaje> garajes, boolean estandar) {
		
		Carrera carrera = null;
		if(estandar) {
			carrera=new Estandar("Dragster");
		}else {
			carrera=new Eliminacion("Dragster");
		}
		
		carrera.setPegatinaTorneo("Fuera de torneo");
		// Si participan varios garajes
		if (garajes.size() > 1) {
			for (Garaje garaje : garajes) {
				Coche coche = garaje.getCoches().get(0 + (int) (Math.random() * garaje.getCoches().size()));
				coche.setCarrera(carrera);//Añadiendo la carrera que es el objeto compartido
				carrera.getCoches().add(coche);
			}
		
		// Si sólo participa un garaje
		} else if (garajes.size() == 1) {
			Garaje garaje = garajes.get(0);
